...
from database.models import UserModule
...
# ================================================================
#  HULPFUNCTIE — volgende toegewezen module bepalen
# ================================================================
def get_next_assigned_module(user_id, track, current_order):
    assigned_slugs = {
        m.module_slug for m in UserModule.query.filter_by(user_id=user_id).all()
    }
    all_modules = load_track_modules(track)
    assigned_in_track = [
        m for m in all_modules
        if m["module_slug"] in assigned_slugs
    ]
    return next(
        (m for m in assigned_in_track if m["order"] > current_order),
        None
    )
...
def testen_content_module(module_name):
    ...
    next_module = get_next_assigned_module(
        user_id, "softwaretesten", lesson["meta"].get("order", 0)
    )

    is_assigned = UserModule.query.filter_by(
        user_id=user_id,
...
def git_content_module(module_name):
    ...
    next_module = get_next_assigned_module(
        user_id, "git", lesson["meta"].get("order", 0)
    )
...
@instructions_bp.route("/developer/content/<module_name>")
@login_required
def developer_content_module(module_name):
    lesson = load_lesson("developer", module_name)
    ...
    next_module = get_next_assigned_module(
        user_id, "developer", lesson["meta"].get("order", 0)
    )
